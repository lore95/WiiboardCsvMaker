...
def read_data():
    ...
    while not stop_event.is_set():
        try:
            ...
            # (keep your median-offset computation at index==20 as-is)
            if(index == 20 ):
                first_10_v1 = [row[1] for row in data_buffer[:10]]
                v1offset = statistics.median(first_10_v1)
                first_10_v2 = [row[2] for row in data_buffer[:10]]
                v2offset = statistics.median(first_10_v2)
                first_10_v3 = [row[3] for row in data_buffer[:10]]
                v3offset = statistics.median(first_10_v3)
                first_10_v4 = [row[4] for row in data_buffer[:10]]
                v4offset = statistics.median(first_10_v4)
            # Apply offsets to channels for zeroing
            v1_adj = v1 + v1offset
            v2_adj = v2 + v2offset
            v3_adj = v3 + v3offset
            v4_adj = v4 + v4offset

            # Compute force from adjusted values (counts → N via quadratic)
            avg_raw = (v1_adj + v2_adj + v3_adj + v4_adj) / 4.0
            F_total = a * (avg_raw ** 2) + b * avg_raw + c
            F_total = float(np.round(F_total, 3))

            with buffer_lock:
                data_buffer.append((index, v1_adj, v2_adj, v3_adj, v4_adj, F_total))
            index += 1
